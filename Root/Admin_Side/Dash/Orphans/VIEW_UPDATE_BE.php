<?php
    ...
    if (isset($_POST['update'])) {
        ...
        if (isset($_POST['org_id']) && !empty($_POST['org_id'])) {
            $org_id = $_POST['org_id'];
            $SQL = "UPDATE orphan_profiles SET org_id = ? WHERE orphan_id = ? LIMIT 1";
        ...
        if (isset($_POST['org_name']) && !empty($_POST['org_name'])) {
            ...
            $SQL = "UPDATE orphan_profiles SET org_name = ? WHERE orphan_id = ? LIMIT 1";
            $stmt = mysqli_prepare($con, $SQL);
            mysqli_stmt_bind_param($stmt, "si", $org_name, $child);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_close($stmt);
        }
