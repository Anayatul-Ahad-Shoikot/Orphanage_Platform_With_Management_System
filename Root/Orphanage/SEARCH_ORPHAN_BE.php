<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['query'])) {
        ...
        $search = $_GET['query'];
        if (!empty($search)) {
            $query = "SELECT * FROM orphan_profiles WHERE (first_name LIKE '%$search%' OR last_name LIKE '%$search%' OR age LIKE '%$search%' OR address LIKE '%$search%' OR gender='$search' OR religion LIKE '%$search%' OR physical_condition LIKE '%$search%' OR education_level LIKE '%$search%' OR medical_history LIKE '%$search%') AND org_name = '$name'";
